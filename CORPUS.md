...
|---|---|---|
| `schemaVersion` | yes | Must be `1`. Anything else loads with a warning. |
| `targetLanguage`, `gameVersion` | no | Reported by `/gubal status`. Metadata only. |
| `npcNames` | no | English → translated speaker names. Only used when **Translate NPC names** is on. |
| `entries[].en` | yes | The English line **as the game renders it**. See below. |
| `entries[].es` | yes | The translation. Entries with either side empty are skipped at load. |
| `entries[].macro` | no | The unresolved macro form. Preferred over `en` for building the key. |
...
the runtime join is **on text**, and `en` must match what the game puts on screen, modulo
...
Do not put asterisks in `es`. The injector marshals plain UTF-8 and does no macro parsing, so
...
1. **Extract.** Read the game's own Excel sheets (via Lumina, off the `sqpack` directory) and emit
   this schema with every `es` empty. Quest sheets, cutscene sheets and the flat ambient sheets
...
3. **Merge.** Emit the runtime file: `en`/`es` only, translated entries only, compact. The editing
...
Paste a recorded key into `corpus.json` as an `en` value and you have an entry guaranteed to match.
